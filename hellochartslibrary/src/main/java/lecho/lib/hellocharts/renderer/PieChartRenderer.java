...
public class PieChartRenderer extends AbstractChartRenderer {
	...
	public PieChartRenderer(Context context, Chart chart, PieChartDataProvider dataProvider) {
		...
		centerCirclePaint2.setAntiAlias(true);
		centerCirclePaint2.setStyle(Paint.Style.STROKE);
	...
	public void initDataAttributes() {
		...
		centerCirclePaint2.setColor(Color.WHITE);
		centerCirclePaint2.setStrokeWidth(data.getCenterCircleWidth());
	...
	private void drawCenterCircle(Canvas canvas) {
		...
		if(data.getCenterCircleWidth()!=0)
		canvas.drawCircle(centerX,centerY,centerRadius*0.9f,centerCirclePaint2);
		...
		if (!TextUtils.isEmpty(data.getCenterText1())) {
			...
			if (!TextUtils.isEmpty(data.getCenterText2())) {
				...
				canvas.drawText(data.getCenterText1(), centerX, centerY - text1Height , centerCircleText1Paint);
				//canvas.drawText(data.getCenterText2(), centerX, centerY + text2Height, centerCircleText2Paint);
				canvas.save();
				Rect rect = new Rect();
				TextPaint textPaint=centerCircleText2Paint;
				textPaint.setColor(data.getCenterText2Color());
				String value=String.valueOf(data.getCenterText2());
				textPaint.getTextBounds(value, 0, value.length(), rect);
				int height = rect.height();
				canvas.translate(centerX,centerY );
				StaticLayout staticLayout1 = new StaticLayout(value
						, textPaint, 600,
