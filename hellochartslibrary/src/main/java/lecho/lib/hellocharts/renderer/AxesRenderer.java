...
/**
 * Default axes renderer. Can draw maximum four axes - two
 * horizontal(top/bottom) and two vertical(left/right).
 * 
 */
public class AxesRenderer {
	...
	/**
	 * Axis positions indexes, used for indexing tabs that holds axes
	...
	/**
	 * Used to measure label width. If label has mas 5 characters only 5 first
	 * characters of this array are used to measure text width.
	 **/
	private static final char[] labelWidthChars = new char[] { '0', '0', '0',
	...
	/**
	 * 4 text paints for every axis, not all have to be used, indexed with TOP,
	 * LEFT, RIGHT, BOTTOM.
	 */
	private Paint[] textPaintTab = new Paint[] { new Paint(), new Paint(),
			new Paint(), new Paint() };
	...
	/**
	 * Holds auto-generated values that should be drawn, i.e if axis is inside
	 * not all auto-generated values should be drawn to avoid overdrawing. Used
	 * only for auto axes.
	 **/
	...
	/**
	 * Buffers for auto-generated values for each axis, used only if there are
	 * auto axes.
	 */
	private AxisAutoValues[] axisAutoValuesBufferTab = new AxisAutoValues[] {
			new AxisAutoValues(), new AxisAutoValues(), new AxisAutoValues(),
			new AxisAutoValues() };

	/**
	 * Holds fixed coordinates for each axis, for horizontal axes Y value is
	 * fixed, for vertical axes X value is fixed.
	 */
	...
	/**
	 * Holds fixed coordinate for axes separations lines, used only for
	 * horizontal axes where fixed value is Y.
	 */
	...
	private FontMetricsInt[] fontMetricsTab = new FontMetricsInt[] {
			new FontMetricsInt(), new FontMetricsInt(), new FontMetricsInt(),
			new FontMetricsInt() };
	...
	public void initAxesAttributes() {
		...
		int axisXTopHeight = initAxisAttributes(chart.getChartData()
				.getAxisXTop(), TOP);
		int axisXBottomHeight = initAxisAttributes(chart.getChartData()
				.getAxisXBottom(), BOTTOM);

		int axisYLeftWidth = initAxisAttributes(chart.getChartData()
				.getAxisYLeft(), LEFT);
		int axisYRightWidth = initAxisAttributes(chart.getChartData()
				.getAxisYRight(), RIGHT);

		chart.getChartComputator().setAxesMargin(axisYLeftWidth,
				axisXTopHeight, axisYRightWidth, axisXBottomHeight);
	}

	/**
	 * Prepare axes coordinates and draw axes lines(if enabled) in the
	 * background.
	 * 
	 * @param canvas
	 */
	...
	public void drawInForeground(Canvas canvas) {
		Axis axis = chart.getChartData().getAxisYLeft();
		if (null != axis) {
			if(axis.getCustomColors()!=null){//睡眠带y轴颜色
				drawAxisVerticalLabels(canvas, axis, LEFT,axis.getCustomColors());
			}else {
	...
	//		drawAxisVerticalLabels(canvas, axis, LEFT);
		...
		if (null != axis) {
			if(axis.getCustomColors()!=null){//睡眠带y轴颜色
				drawAxisVerticalLabels(canvas, axis, RIGHT,axis.getCustomColors());
			}else {
	 ...
	 * Returns axis measured width( for left and right) or height(for top and
	 * bottom).
	 */
	private int initAxisAttributes(Axis axis, int position) {
		...
		textPaintTab[position].setTextSize(Utils.sp2px(scaledDensity,
				axis.getTextSize()));
		textPaintTab[position].getFontMetricsInt(fontMetricsTab[position]);

		axisLabelTextAscentTab[position] = Math
				.abs(fontMetricsTab[position].ascent);
		axisLabelTextDescentTab[position] = Math
				.abs(fontMetricsTab[position].descent);
		axisLabelWidthTab[position] = (int) textPaintTab[position].measureText(
				labelWidthChars, 0, axis.getMaxLabelChars());
		...
		if (LEFT == position || RIGHT == position) {
			...
			// If auto-generated or has manual values add height for value
			// labels.
			...
			throw new IllegalArgumentException("Invalid axis position: "
					+ position);
		}
	...
	private void prepareAxis(Axis axis, int position) {
		...
		if (BOTTOM == position) {
			...
			if (axis.isInside()) {
				axisFixedCoordinateTab[position] = computator
						.getContentRectWithMargins().bottom
						- axisMargin
						- axisLabelTextDescentTab[position];
				axisNameBaselineTab[position] = computator
						.getContentRectWithMargins().bottom
				...
				axisFixedCoordinateTab[position] = computator
						.getContentRectWithMargins().bottom
						+ axisLabelTextAscentTab[position] + axisMargin;
				axisNameBaselineTab[position] = axisFixedCoordinateTab[position]
						+ axisMargin
			...
			if (axis.isInside()) {
				...
				axisFixedCoordinateTab[position] = computator
						.getContentRectWithMargins().left + axisMargin;
				axisNameBaselineTab[position] = computator
						.getContentRectWithMargins().left
						- axisMargin
						- axisLabelTextDescentTab[position];
			} else {
			...
			if (axis.isInside()) {
				textPaintTab[position].setTextAlign(Align.RIGHT);
				axisFixedCoordinateTab[position] = computator
						.getContentRectWithMargins().right - axisMargin;
				axisNameBaselineTab[position] = computator
						.getContentRectWithMargins().right
						+ axisMargin
						+ axisLabelTextAscentTab[position];
			} else {
				textPaintTab[position].setTextAlign(Align.LEFT);
			...
			throw new IllegalArgumentException(
					"Invalid position for horizontal axis: " + position);
		}
	...
	private void prepareAxisHorizontalCustom(Axis axis, int position) {
		...
		Log.i(TAG, "prepareAxisHorizontalCustom: "+scale);
		float maxScale=axis.getScale();
		int module = (int) Math
				.ceil((axis.getValues().size() * axisLabelWidthTab[position])
	...
	//	if(scale!=1&&maxScale!=0&&scale<maxScale)module=1;

		if (axis.hasLines()
				&& axisLinesDrawBufferTab[position].length < axis.getValues()
						.size() * 4) {
			axisLinesDrawBufferTab[position] = new float[axis.getValues()
					.size() * 4];
		}

		if (axisRawValuesTab[position].length < axis.getValues().size()) {
			axisRawValuesTab[position] = new float[axis.getValues().size()];
			axisValuesToDrawTab[position] = new AxisValue[axis.getValues()
					.size()];
		}
		...
		for (AxisValue axisValue : axis.getValues()) {
			...
			if (value >= visibleViewport.left && value <= visibleViewport.right) {
				...
				// Draw axis values that have 0 module value, this will hide
				// some labels if there is no place for them.
				if (0 == valueIndex % module) {
					
					final float rawX = computator.computeRawX(axisValue
							.getValue());
	...
	private void prepareAxisHorizontalAuto(Axis axis, int position) {
		...
		Utils.computeAxisAutoValues(visibleViewport.left,
				visibleViewport.right, contentRect.width()
						/ axisLabelWidthTab[position] / 2,
				axisAutoValuesBufferTab[position]);
		...
		for (int i = 0; i < axisAutoValuesBufferTab[position].valuesNumber; ++i) {
			...
			final float rawX = computator
					.computeRawX(axisAutoValuesBufferTab[position].values[i]);
	...
	private void drawAxisHorizontalLines(Canvas canvas, Axis axis, int position) {
		final Rect contentRectMargins = chart.getChartComputator()
				.getContentRectWithMargins();

		// Draw separation line with the same color as axis text. Only
		// horizontal axes have separation lines.
		if (axis.hasAxisLines()) {
			canvas.drawLine(contentRectMargins.left,
					axisSeparationLineTab[position], contentRectMargins.right,
		...
		if(axis.getDrawSize()!=0) drawCircleAxis(canvas,linePaint,axis);

//		canvas.drawLines(axisLinesDrawBufferTab[position], 0,
//				valueToDrawIndex * 4, linePaint);
	}

	private void drawCircleAxis(Canvas canvas,Paint paint,Axis axis) {
		ChartComputator chartComputator=chart.getChartComputator();
		final Viewport visibleViewport = chartComputator.getVisibleViewport();
		final Rect contentRectMargins =chartComputator
				.getContentRectWithMargins();
		Path path=new Path();
		int size=axis.getDrawSize();
		float radiusNormal= Utils.dp2px(density,5);
		float radiusSmall= Utils.dp2px(density,3);
		for(int i=0;i<size;i++){
			float radius=i==0||i==size-1?radiusNormal:radiusSmall;
			if (i >= visibleViewport.left && i <= visibleViewport.right){
				final float rawX = chartComputator.computeRawX(i);
				if(checkRawX(contentRectMargins, rawX, axis.isInside(), i))
					path.addCircle(rawX,contentRectMargins.bottom,radius, Path.Direction.CW);
			}
	...
	private void drawAxisHorizontalLabels(Canvas canvas, Axis axis, int position) {
		...
		for (; valueToDrawIndex < axisValuesToDrawNumTab[position]; ++valueToDrawIndex) {
			...
			if (axis.isAutoGenerated()) {
				...
				numChars = axis.getFormatter().formatAutoValue(labelBuffer,
						valuesBuff, axisAutoValuesBufferTab[position].decimals);
			} else {
				valuesBuff[0] = axisValuesToDrawTab[position][valueToDrawIndex]
						.getValue();
				numChars = axis.getFormatter().formatValue(
						labelBuffer,
						valuesBuff,
						axisValuesToDrawTab[position][valueToDrawIndex]
								.getLabel());
			}

							canvas.drawText(labelBuffer, labelBuffer.length - numChars,
					numChars, axisRawValuesTab[position][valueToDrawIndex],
				axisFixedCoordinateTab[position], textPaintTab[position]);
		...
		if (!TextUtils.isEmpty(axis.getName())) {
			textPaintTab[position].setTextAlign(Align.CENTER);
			canvas.drawText(axis.getName(), contentRectMargins.centerX(),
					axisNameBaselineTab[position], textPaintTab[position]);
		}
	}

	/**
	 * For axis inside chart area this method checks if there is place to draw
	 * axis label. If yes returns true, otherwise false.
	 */
	private boolean checkRawX(Rect rect, float rawX, boolean axisInside,
			int position) {
		if (axisInside) {
	...
	private void prepareAxisVerticalCustom(Axis axis, int position) {
		...
		int module = (int) Math.ceil((axis.getValues().size()
				* axisLabelTextAscentTab[position] * 2)
	...
	private void prepareAxisVerticalAuto(Axis axis, int position) {
		...
		Utils.computeAxisAutoValues(visibleViewport.bottom,
				visibleViewport.top, contentRect.height()
						/ axisLabelTextAscentTab[position] / 2,
				axisAutoValuesBufferTab[position]);
		...
		for (int i = 0; i < axisAutoValuesBufferTab[position].valuesNumber; i++) {
			final float rawY = computator
					.computeRawY(axisAutoValuesBufferTab[position].values[i]);
	...
	private void drawAxisVerticalLines(Canvas canvas, Axis axis, int position) {
		...
		if (axis.hasAxisLines()) {
			...
			canvas.drawLine(axisSeparationLineTab[position],
					contentRectMargins.bottom, axisSeparationLineTab[position],
					contentRectMargins.top, textPaintTab[position]);
		}
		if (!axis.hasLines()) {
			return;
		}
		...
		for (; stopsToDrawIndex < axisValuesToDrawNumTab[position]; ++stopsToDrawIndex) {
			...
			path.moveTo(contentRectMargins.left,axisRawValuesTab[position][stopsToDrawIndex]);
			path.lineTo(contentRectMargins.right,axisRawValuesTab[position][stopsToDrawIndex]);
		}

		linePaint.setColor(axis.getLineColor());
		if(axis.isDashPath()){

			DashPathEffect dashPathEffect=new DashPathEffect(new float[]{20,4},0);
			linePaint.setPathEffect(dashPathEffect);
			canvas.drawPath(path,linePaint);
		}else{

			canvas.drawLines(axisLinesDrawBufferTab[position], 0,
					stopsToDrawIndex * 4, linePaint);
		}
	...
	private void drawAxisVerticalLabels(Canvas canvas, Axis axis, int position) {
		...
		for (; stopsToDrawIndex < axisValuesToDrawNumTab[position]; ++stopsToDrawIndex) {
			...
			if (axis.isAutoGenerated()) {
				...
				valuesBuff[0] = axisValuesToDrawTab[position][stopsToDrawIndex]
						.getValue();

				numChars = axis.getFormatter().formatValue(
						labelBuffer,
						valuesBuff,
						axisValuesToDrawTab[position][stopsToDrawIndex]
								.getLabel());
			}

			canvas.drawText(labelBuffer, labelBuffer.length - numChars,
					numChars, axisFixedCoordinateTab[position],
					axisRawValuesTab[position][stopsToDrawIndex],
		...
		if (!TextUtils.isEmpty(axis.getName())) {
			textPaintTab[position].setTextAlign(Align.CENTER);
			canvas.save();
			canvas.rotate(-90, contentRectMargins.centerY(),
					contentRectMargins.centerY());
			canvas.drawText(axis.getName(), contentRectMargins.centerY(),
					axisNameBaselineTab[position], textPaintTab[position]);
			canvas.restore();
		}
	...
	private void drawAxisVerticalLabels(Canvas canvas, Axis axis, int position,int[] colors) {
		final Rect contentRectMargins = chart.getChartComputator()
				.getContentRectWithMargins();
		...
		for (; stopsToDrawIndex < axisValuesToDrawNumTab[position]; ++stopsToDrawIndex) {
			...
			textPaintTab[position].setColor(colors[stopsToDrawIndex]);
			canvas.drawText(labelBuffer, labelBuffer.length - numChars,
					numChars, axisFixedCoordinateTab[position],
					axisRawValuesTab[position][stopsToDrawIndex],
					textPaintTab[position]);
	 ...
	 * axis label. If yes returns true, otherwise false.
	 */
	private boolean checkRawY(Rect rect, float rawY, boolean axisInside,
			int position) {
		if (axisInside) {
			float marginBottom = axisLabelTextAscentTab[BOTTOM] + axisMargin;
			float marginTop = axisLabelTextAscentTab[TOP] + axisMargin;
			if (rawY <= rect.bottom - marginBottom
					&& rawY >= rect.top + marginTop) {
				return true;
			} else {
				return false;
			}
	...
	private void drawBgChart(Canvas canvas) {
		final ChartComputator computator = chart.getChartComputator();
		final float rawX = computator.computeRawX(0);
		float columnWidth=calculateColumnWidth();
		final float halfColumnWidth = columnWidth / 2;
		final float subcolumnSpacing =Utils.dp2px(density, 1);
		float subcolumnRawX = rawX ;
		float subcolumnWidth = (columnWidth - subcolumnSpacing )/2;
		List<AxisValue>axisValues=chart.getChartData().getAxisXBottom().getValues();
		for(int i=0;i<axisValues.size();i++){
			AxisValue axisValue=axisValues.get(i);
			canvas.drawText(axisValue.getLabel(),0,axisValue.getLabel().length,rawX+halfColumnWidth/2+i*9*(subcolumnWidth+subcolumnSpacing),axisFixedCoordinateTab[BOTTOM],textPaintTab[BOTTOM]);
		}
	}
