...
public class ChartTouchHandler {
	 ...
	 * 
	 * Using first approach of fling animation described here {@link
	 * http://developer.android.com/training/custom-views/making-interactive.html}. Consider use of second option with
	 * ValueAnimator.
	 * 
	 */
		...
		if (null != viewParent) {
			if (ContainerScrollType.HORIZONTAL == containerScrollType && !scrollResult.canScrollX
				...
				viewParent.requestDisallowInterceptTouchEvent(false);
					...
					&& !scaleGestureDetector.isInProgress()) {
				viewParent.requestDisallowInterceptTouchEvent(false);
			}else if (ContainerScrollType.HORIZONTAL_IN_ScrollView == containerScrollType && !scrollResult.canScrollX
					&& !scaleGestureDetector.isInProgress()) {
				viewParent.requestDisallowInterceptTouchEvent(false);
			}
		}
	...
	protected class ChartGestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			if (isScrollEnabled) {
				boolean canScroll;
				if(containerScrollType==ContainerScrollType.HORIZONTAL_IN_ScrollView){
					canScroll = chartScroller
							.scrollInScrollView(chart.getChartComputator(), distanceX, distanceY, scrollResult);
				}else {
					canScroll = chartScroller
